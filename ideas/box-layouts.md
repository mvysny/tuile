...
- **Hand-rolled `Fill`.** `[inner.height - 8, 2].max` (`:691`), same idiom at
  `:173`, `:317`, `:349`, `:571`.
...
|---|---|
| `Fixed(n)`, `Percent(n)`, `Fill(weight)` | `Auto` / `Pack` / shrink-to-fit |

urwid's `PACK`, CSS `auto`, FTXUI's non-`flex` default and ratatui's
content-derived sizing all sit on the wrong side. **This single omission is
what keeps the feature sugar rather than a reopened wound.**
...
1. **Default main-axis constraint: `Fixed(1)`.** Matches Vaadin 8's
   ...
   every field is one row tall. (Rejected `Fill(1)`: "bare `Vertical` evenly
   splits its rows" is tidier as a container story but wrong for what people
   will actually build.)
2. **Default cross-axis constraint: `Percent(100)`**, and **`Fill` is
   ...
   meaningful only against sibling weights. `Fill(1)` as the cross default
   would advertise a negotiation that cannot happen, and leaves "what does
   `Fill(2)` mean on the cross axis?" merely undocumented instead of
...
3. **`spacing` and `padding` are layout-global, not per-child.** Beyond
   ...
   conventions exist in the wild and both confuse.
4. **Percent *and* Fill resolve against space that is actually available** —
...
6. **Remainder from weighted `Fill`: distributed one cell at a time,
...
Naive "last `Fill` absorbs everything left over" is tempting — it matches the
hand-written idiom in ch3 and `Sampler#rect=` (`left_w`, then
`rect.width - left_w`), and guarantees an exact sum structurally. **But it
degrades badly with more than two children:** five equal `Fill`s in 12 rows
...
earliest `Fill` children, one each."*
...
cleaned up in the existing `Layout#remove`. Against AGENTS.md's slot-desync
rule: this is a *per-child attribute map, not a second copy of ordering* —
`@children` remains the sole ordering authority — so it doesn't trip the rule
the way `ScreenPane#popups` does.
...
`child.layout_constraint = Fill(1)` is the tempting third option and is a
...
Bare `Fixed[4]` / `Fill[1]` **resolve inside a user's `Layout::Vertical`
...
```
lib/tuile/component/layout.rb              + nested Fixed / Percent / Fill / Insets
lib/tuile/component/layout/box.rb          Layout::Box — the shared 1-D pass (abstract)
...
junk drawer. `Box` stays abstract — no `axis:` parameter, because
...
4. `Fill(w)` children split the residue by weight; remainder one cell at a
   time, leftmost-first.
...
- **No `Fill` present:** slack is left at the end (pack from start). This is
...
- **`Fill(0)` or negative weight:** raise at declaration. A silent zero-extent
  child is a miserable debugging experience.
...
`add(@right_window, Fill(1))` — but that **loses** `(rect.width / 3).clamp(20, 40)`,
which is a `Min` + `Max` around a `Percent` and unsayable in a three-constraint
vocabulary. Fine: that's a one-line `Absolute` override, and it's better than
adding `Min`/`Max` on day one. But it is honest evidence that `Horizontal`
serves *forms* better than it serves *app shells*.
...
- `Insets` shape: `Data.define(:top, :right, :bottom, :left)` with
  `Insets[top: 1]` keyword defaults plus an Integer coercion for uniform?
  Or skip the type and take `padding: 1` / `padding_top: 1` keywords? Leaning
  the value type — it's one nested `Data.define` and it composes.
- Does `Percent` on the **cross** axis pull its weight, given `Fixed` clamps
  and `Percent(100)` is the default? Possibly YAGNI; cheap to keep, cheap to
  drop.
~~Whether `Layout::Box` should be public API at all.~~ **Settled: public.**
It's the shared 1-D pass *and* the home for the common calculation helpers
(available-extent arithmetic, weighted remainder distribution, cross-axis
placement), so a future third box-shaped layout extends it rather than
re-deriving them.
...
- **book ch3:** rewrite the deferral paragraph into a real section
  (`Length` → `Fixed`, the two-child remainder clause, the nesting-for-groups
  idiom), and tighten the "validated by the ecosystem" sentence per above.
- **AGENTS.md:** the no-`Auto` invariant; "alignment needs a width, not the
  child's width"; `Fill` is main-axis-only; the constraint map is an attribute
  map and not an ordering copy; relayout no-ops while `rect.empty?`.
- **DECISIONS.md `D-box-layouts`:** decisions 1-6 with their rejected
  alternatives — especially the remainder-distribution three-way, the
  constraint-on-`Component` reject, and the block-form-alignment reject.
