...
| ~~DateField~~ | `TextField` + `HasBadInput` | **built** 2026-09-04 (`D_date_field`, book ch7); manual entry only, stdlib `Date`, several strftime formats (first match wins and writes back). Calendar popup is phase 2 |
...
| Calendar grid for `DateField`, and the Time / DateTime twins | the grid needs the calendar popup over Popover (L) — phase 2 of `D_date_field` |
