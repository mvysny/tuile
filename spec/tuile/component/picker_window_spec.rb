...
module Tuile
  describe Component::PickerWindow do
    ...
    it "smokes" do
      w = Component::PickerWindow.new("foo", [%w[a all]]) {}
      screen.add_popup w
      w.close
    end

    it "opens as popup" do
      w = Component::PickerWindow.open("foo", [%w[a all]]) {}
      assert w.open?
      w.close
    end

    it "doesnt call block if closed" do
      w = Component::PickerWindow.new("foo", [%w[a all]]) { raise "should not be called" }
      screen.add_popup w
      w.handle_key("q")
      assert !w.open?
    end
    ...
    it "selects first option on enter" do
      selected = nil
      w = Component::PickerWindow.new("foo", [%w[a all]]) { selected = it }
      screen.add_popup w
