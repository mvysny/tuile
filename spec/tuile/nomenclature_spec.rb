...
module Tuile
  RSpec.describe "nomenclature" do
    ...
    # `lib/` is a reader generated by `attr_accessor` — the override point is
    # `handle_foo`. A hand-written `def on_foo` would be one of the two halves
    # of the collision `D_handler_naming` deleted: it either shadows the
    # generated reader, or turns an in-class `on_foo&.call` into a call of the
    # *hook*, which fires the slot and then `&.call`s whatever the app's lambda
    # returned. Both are silent. A writer (`def on_foo=`) is fine — three of
    # them forward to an inner widget. No allowlist, as above.
