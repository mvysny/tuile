...
module Tuile
  describe VerticalScrollBarInk do
    context "height == 0" do
      it "constructor succeeds" do
        assert VerticalScrollBarInk.new(0, row_count: 0, scroll_top_row: 0)
      end

      it "does not set handle instance variables" do
        sb = VerticalScrollBarInk.new(0, row_count: 0, scroll_top_row: 0)
    ...
    context "height == 1" do
      let(:sb) { VerticalScrollBarInk.new(1, row_count: 10, scroll_top_row: 0) }
    ...
    context "content fits (row_count <= height)" do
      let(:sb) { VerticalScrollBarInk.new(5, row_count: 3, scroll_top_row: 0) }
      ...
      it "draws no handle when the content exactly fills the viewport either" do
        exact = VerticalScrollBarInk.new(5, row_count: 5, scroll_top_row: 0)
        (0..4).each { |r| assert_equal "░", exact.scrollbar_char(r) }
      end

      it "draws the handle again as soon as one row overflows" do
        overflowing = VerticalScrollBarInk.new(5, row_count: 6, scroll_top_row: 0)
    ...
    context "content overflows: 20 lines, height 10, scroll_top_row 0" do
      let(:sb) { VerticalScrollBarInk.new(10, row_count: 20, scroll_top_row: 0) }
    ...
    context "content overflows: 20 lines, height 10, scroll_top_row 10" do
      let(:sb) { VerticalScrollBarInk.new(10, row_count: 20, scroll_top_row: 10) }
    ...
    context "glyphs" do
      ...
      it "defaults to the block glyphs" do
        assert_equal "█", VerticalScrollBarInk.handle_char
        assert_equal "░", VerticalScrollBarInk.track_char
      end

      it "draws whatever the app assigned" do
        VerticalScrollBarInk.handle_char = "▐"
        VerticalScrollBarInk.track_char = "│"
        sb = VerticalScrollBarInk.new(10, row_count: 20, scroll_top_row: 0)
        assert_equal "▐", sb.scrollbar_char(0)
