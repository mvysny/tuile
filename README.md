...
| `FormItem` | One row of a form: a `caption` above a field, an optional required marker beside it, and the field's `error_message` mirrored into the row below. Three rows that never reflow — the message row doubles as the gap. Hide the item, not the field. |
