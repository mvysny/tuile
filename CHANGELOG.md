...
- Fix `Component::List#auto_scroll` ignoring a wheel notch or a PgUp: those went through `move_scroll_top_row_by`, which wrote the scroll row behind `scroll_top_row=`'s back and left `following?` armed, so the next incoming row yanked the viewport back to the tail. It routes through the setter now, as `TextView` already did.
- Fix `Component::List`'s scrollbar being ink you could not touch: a visible one is now a real `Component::VerticalScrollBar` child in the rightmost column, so its handle drags (with `run_event_loop(capture_mouse: :drag)`) and its track pages. The handle also sits where the component places it, up to a row from where `VerticalScrollBarInk` drew it. See `design/decisions.md` `D_draggable_scrollbar`.
