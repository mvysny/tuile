...
- Add `Tuile::Mouse` — one class per mouse event, `DownEvent` / `UpEvent` / `ScrollEvent` / `MoveEvent` off the wire plus the router-made `DragEvent`, each including the `Mouse::Event` marker and its `point`; `Mouse.parse` replaces `MouseEvent.parse`. See `design/decisions.md` `D_mouse_dispatch`.
