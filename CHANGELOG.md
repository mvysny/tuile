...
- Add `Component::Popup#close_on_outside_click?` — a left click that misses an open popup now closes it, modal or not (default true; `Component::Notification` opts out). Fixes dropdowns and menu cascades stranded by a click on inert decoration. See `DECISIONS.md` `D-outside-click`.
