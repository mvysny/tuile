...
- **Breaking:** `Component::ComboBox` no longer includes `Component::HasContent` — its inner `TextField` holds a transient query it owns, not content you populate — so `combo.content` and `combo.content=` are gone. A spec reaches the field with `Testing.get(Component::TextField, in: combo)`. See `design/decisions.md` `D_wrapping_field`.
