...
- **Breaking:** an overlay's placement must now include `Component::Overlay::Placement` and answer `rect_for(overlay, screen_size, anchor_rect)` — the duck type is named, documented and checked where the app hands it over, instead of failing as a `NoMethodError` inside the pane a settle later. Add the `include` and a third parameter; a placement that hangs off a widget answers `anchor` with the component (or a screen `Rect`, or `nil` for none) and the pane resolves it, so the visibility walk, the `attached?` test and `absolute_extent_rect` are no longer yours to replicate.
