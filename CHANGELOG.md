...
- Add `Component::HasValidation` — `error_message` plus `on_error_message_change`, the verdict slot a validator writes and the field paints in the new `Theme#error_color`; included by `HasValue`, so every field has it. See `DECISIONS.md` `D_has_validation` and book ch7.
- Add `Component#content_fg_color` — a protected hook declaring the color a widget paints its content in, resolved down the tree by `draw_text` / `draw_char`; how an invalid field's ink reaches a composed field's inner widget without being forwarded.
- Add `StyledString#under_fg` — sets `fg` only on spans that have none, the fill-unset counterpart of `with_fg` and the mirror of `under_bg`.
...
- **Breaking:** `Theme` carries two further tokens, `scrollbar_color` and `error_color`, so `Component::List` and `TextView` no longer paint their scrollbar in the terminal's default foreground — a near-white column on a dark scheme — and a field can paint a validation verdict. A hand-rolled `Theme.new(...)` must now pass both; `Theme::DARK.with(...)` is unaffected. See `DECISIONS.md` `D_scrollbar_ink` and `D_has_validation`.
