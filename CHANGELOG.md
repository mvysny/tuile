...
- **Breaking:** `Component::Button#caption` returns a `StyledString`, not a `String`. Callers measuring it must use `caption.display_width` (the natural button width is `caption.display_width + 4`); `caption.to_s` recovers the plain text.
