...
- Add `Component#clip_rect` — the region of its own coordinate space a container allows its **descendants** to paint in, `nil` (the default) imposing none; ancestors' intersect, a write outside is dropped and a row crossing an edge sliced to it. See `design/decisions.md` `D_clip`.
- Add `Tuile::Canvas#clip` — the backend-space region a canvas may write to, built by `Screen#canvas_for` from the ancestors' `clip_rect`s and carried through `with(bg_color:)`; `nil` is unclipped, and a caret outside it hides the cursor. See `design/decisions.md` `D_clip`.
