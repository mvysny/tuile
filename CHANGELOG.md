...
- Add `Testing.click` / `Testing.set_value` — gestures that drive a located component only where a user could have: `click` routes a real press at the cell the component paints and raises when a modal, a popup or a hidden ancestor would have swallowed it; `set_value` refuses a field outside the key scope and moves no focus.
...
- **Breaking:** `Testing::LookupError` is gone; every lookup and gesture raises `Testing::AssertionError`, which descends from `Exception` rather than `Tuile::Error` because a failed assertion is not production's error and nobody should be catching one. Rename it in `assert_raises`, and replace a `rescue Tuile::Error` around a lookup with the class itself.
