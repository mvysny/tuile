...
- Add `Component::HasBadInput` — `bad_input?` / `bad_input_message`, the report a field owes when its input is something its value cannot represent (a lone `-` in an `IntegerField` reads `nil`, exactly like an empty one). Included by the three numeric fields and `DateField`; ask it before `empty?`. See `DECISIONS.md` `D_bad_input` and book ch7.
...
- Add `Component::HasPlaceholder` — `placeholder=` paints a hint into a field's empty well (`dd.mm.yyyy`) in the new barely-visible `Theme#placeholder_color`, never touching the value; carried by `TextField`, forwarded by the composed fields. See `DECISIONS.md` `D_placeholder` and book ch7.
- Add `Component::DateField` — a one-row field whose `value` is a `Date` or `nil`, accepting several strftime formats (first match wins, first writes back), canonicalizing on blur and Enter. See `DECISIONS.md` `D_date_field` and book ch7.
- Add `Component::AbstractWrappingField` — the base for a field that carries a typed value but paints nothing itself, owning and hiding one inner editor; `IntegerField`, `FloatField`, `BigDecimalField` and `DateField` are built on it. See `DECISIONS.md` `D_wrapping_field`.
