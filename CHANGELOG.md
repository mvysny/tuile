...
- Add `Component::DateField.default_format` / `.default_calendar_start` — the app-global seeds each field reads at construction, defaulting to `"%Y-%m-%d"` and `Date::GREGORIAN` rather than Ruby's `Date::ITALY`. Both are marked as possibly changing once a locale seam exists.
