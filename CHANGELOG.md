...
- Add `Component::IntegerField` — a single-line input whose `value` is an `Integer` (or `nil` when empty/un-parseable). Accepts only `0`–`9` and a single leading `-`; any other printable key is rejected without moving the caret. Composes a `TextField` (never subclasses one, so its face carries only the typed `HasValue` seam, not the widget's String `text`); `on_value_change` fires eagerly, once per real value change. No `min`/`max`, `+`, grouping, or spinner (a forms concern).
