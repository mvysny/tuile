...
- Fix `Component::List`'s scrollbar being ink you could not touch: a visible one is a `Component::VerticalScrollBar` child now, so its handle drags (wanting `run_event_loop(capture_mouse: :drag)`) and its track pages, and it sits up to a row from where `VerticalScrollBarInk` drew it. See `design/decisions.md` `D_draggable_scrollbar`.
