...
- Add `Tuile::Canvas` — the surface a component paints onto, handed to `Component#repaint` and passed on to `#draw_text` / `#draw_char` / `#clear_background`. `Canvas::Direct`, the only implementation, writes straight into `Screen#buffer`, which the new `Screen#canvas` wraps. See `design/decisions.md` `D_canvas`.
...
- **Breaking:** `Component#repaint` takes the `Tuile::Canvas` to paint onto, and `#draw_text` / `#draw_char` / `#clear_background` / `#clear_outside_extent` / `#clear_inside_extent` take it as their first argument. An override becomes `def repaint(canvas = screen.canvas)` and threads `canvas` into each of those calls; `super` forwards it unchanged.
