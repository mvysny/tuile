...
- Fix `Component#invalidate_layout` to drop a mark made during the container's own `relayout`, so hiding or adding a child there no longer costs a second pass or raises a false stale-rect error on a freshly built tree. See `design/decisions.md` `D_deferred_layout`.
