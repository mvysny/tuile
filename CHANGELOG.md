...
- Add `Testing.place(component, rect)` — sizes a component through whatever places it, since `rect=` raises outside the parent's `relayout`: a parentless root in a throwaway holder, an `Absolute` child by its constraint, an open overlay by `At[rect]`.
