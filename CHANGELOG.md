...
- **Breaking:** `Component#on_tree` and `#on_shown_tree` are now `#walk_tree` and `#walk_shown_tree` — the `on_` prefix is reserved for hooks and listener slots, and these are traversals, not events. Rename the call; nothing else about either walk changed.
