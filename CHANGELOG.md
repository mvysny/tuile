...
- **Breaking:** `Theme` carries a fifth token, `scrollbar_color`, so `Component::List` and `TextView` no longer paint their scrollbar in the terminal's default foreground — a near-white column on a dark scheme. A hand-rolled `Theme.new(...)` must now pass it; `Theme::DARK.with(...)` is unaffected. See `DECISIONS.md` `D_scrollbar_ink`.
