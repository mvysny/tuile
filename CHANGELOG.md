...
- Add `Component::AbstractWrappingField` — the base for a field that carries a typed value but paints nothing itself, owning and hiding one inner editor; `IntegerField`, `FloatField` and `BigDecimalField` are built on it. It commits on both commit gestures (leaving the focus chain, and ENTER, which it leaves bubbling so a form's default button still fires) and offers subclasses an `on_editor_change` hook for the editor's claimed `on_change`. See `DECISIONS.md` `D_wrapping_field`.
- Add `Component::DateField` — a typed `Date`/`nil` field over a list of strftime `formats` taken from `Screen#locale` until you set them: parsing tries them in order and leaving the field rewrites the buffer in the first, Up/Down step a day, and unparseable input is reported through `bad_input?` rather than filtered. Its red well waits for a commit gesture, so typing `2` toward `2026-09-04` never reddens. See `DECISIONS.md` `D_date_field` and book ch7.
- Add `Component::TimeField` — a typed `Time`/`nil` field holding a time of day on a fixed epoch date, spelled the way `Screen#locale` says: `step` is both the Up/Down stride and the precision, showing seconds only below a minute, PageUp/PageDown step an hour, and unparseable input is reported through `bad_input?` rather than filtered. See `DECISIONS.md` `D_time_field` and book ch7.
...
- `examples/sampler.rb` gains a `DateField` pane under Input → Typed, where typing `4.9.2026` and Tabbing away shows the buffer rewrite.
- Add readline's kill keys to the text inputs: `Ctrl+W` deletes the word behind the caret (`AbstractStringField`, so both inputs), `Ctrl+U` everything before it — back to index 0 in a `TextField`, to the caret's row start in a `TextArea`. A `ComboBox` query now clears in one press, at the cost of the dropdown's `Ctrl+U` half-page scroll. See `DECISIONS.md` `D_kill_keys` and book ch7.
...
- **Breaking:** `Theme` carries four further tokens — `scrollbar_color`, plus `error_color` / `error_bg_color` / `error_active_bg_color` — so `Component::List` and `TextView` no longer paint their scrollbar in the terminal's default foreground (a near-white column on a dark scheme), and a field can show a validation verdict as a well that still reads as focused. A hand-rolled `Theme.new(...)` must now pass all four; `Theme::DARK.with(...)` is unaffected. See `DECISIONS.md` `D_scrollbar_ink` and `D_has_validation`.
...
- **Breaking:** `content` / `content=` are gone from `Component::IntegerField`, `FloatField` and `BigDecimalField` — their inner editor is private machinery, and a public setter that swapped it left the widget permanently broken. An app uses the field's own `placeholder` / `on_enter` / `cursor_position` / `clear`, and needs a forwarder for anything else; a *spec* reaches the editor with `Testing.get(Component::TextField, in: field)`. See `DECISIONS.md` `D_wrapping_field`.
