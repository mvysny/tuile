...
- Add `Component#extent`, `#extent_rect` and `#clear_outside_extent` — the size a widget actually paints inside the rect it was assigned (`nil` by default, meaning undeclared). Declaring one narrows what the default `repaint` blanks, and is what the widget hit-tests and anchors against; `rect` still means exactly what the parent assigned. See `DECISIONS.md` `D_extent`.
