...
- Add `Component::DateField` — a typed `Date`/`nil` field over a list of strftime `formats`: parsing tries them in order and leaving the field rewrites the buffer in the first, Up/Down step a day, and unparseable input is reported through `bad_input?` rather than filtered. See `DECISIONS.md` `D_date_field` and book ch7.
