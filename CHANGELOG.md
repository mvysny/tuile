...
- Add `Component#clip_rect` — the region of its own coordinate space a component allows its **descendants** to paint in, defaulting to `local_rect`, so a write past the rect a component was given is now dropped rather than landing on a neighbour; override to narrow it, `nil` to impose nothing of your own. See `design/decisions.md` `D_clip`.
- Add `Tuile::Canvas#clip` — the backend-space region a canvas may write to, built by `Screen#canvas_for` from the ancestors' `clip_rect`s and carried through `with(bg_color:)`; a row crossing an edge is sliced, a glyph the edge falls inside blanked rather than split, and a caret outside it hides the cursor. See `design/decisions.md` `D_clip`.
