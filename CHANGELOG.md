...
- Add `Tuile::Canvas` — the surface a component paints onto, handed to `Component#repaint` and threaded into `#draw_text` / `#draw_char` / `#clear_background`, with the sole implementation `Canvas::Direct` writing into `Screen#buffer` as the new `Screen#canvas` does. See `design/decisions.md` `D_canvas`.
