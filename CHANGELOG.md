...
- Add `Tuile::Mouse` — one class per wire event (`DownEvent`, `UpEvent`, `ScrollEvent`, `MoveEvent`, and the router-made `DragEvent`), sharing the `Mouse::Event` marker module and its `point`, with `Mouse.parse` replacing `MouseEvent.parse`. See `design/decisions.md` `D_mouse_dispatch`.
...
- Add `capture_mouse: :drag` / `:hover` to `Screen#run_event_loop`, each rung unlocking one tier of events: `:drag` (mode 1002) adds `handle_mouse_drag`, `:hover` (1003) adds `handle_mouse_move?` and the enter/exit hooks. `true` still means `:clicks`, and an unknown level now raises. See `design/research.md` `R_mouse_reporting`.
- Add `FakeScreen#click` / `#press` / `#release` / `#scroll` / `#move` — a spec posts the gesture the terminal would, at screen coordinates, instead of calling a handler.
...
- **Breaking:** `Component#handle_mouse` is gone, and with it the walk every override had to `super` into: a component overrides one handler per event — `handle_mouse_down?` / `handle_mouse_scroll?` / `handle_mouse_move?` answering `true` to claim, `handle_mouse_up` / `handle_mouse_drag` / `handle_mouse_enter` / `handle_mouse_exit` answering nothing. Move an old body into `handle_mouse_down?`, drop the `super` and the hit test the router now does, and return `true` when you acted. See `design/decisions.md` `D_mouse_dispatch`.
...
- **Breaking:** Click-to-focus moved into `Mouse::Router`, which focuses the innermost `focusable?` under the pointer before any handler runs, so a widget can no longer break focus by forgetting `super`. Delete a widget's own `extent_rect` hit test: a press only reaches it where the extent contains the point.
...
- **Breaking:** `Component#handle_paste` returns `void` rather than a Boolean — a paste reaches the focused component and stops, so one that declines has nowhere to hand it on to and there was never a verdict to carry. Drop the trailing `true` from an override; nothing read the answer. See `design/decisions.md` `D_bracketed_paste`.
...
- **Breaking:** `COMPARISON.md` and `design/requirements.md` are retired — the toolkit survey is now `design/research.md`'s `R_ruby_tui_toolkits` / `R_ratatui` / `R_charm_ruby` / `R_curses_bindings` entries, and the one requirement is the **Promises** section of `AGENTS.md`. `R_` slugs now name research rather than requirements, and every `design/decisions.md` heading is a question; `CLAUDE.md` is a symlink to `AGENTS.md` rather than a one-line shim.
- **Breaking:** Remove `Theme#hint_color` and `Theme#hint` — the token lost its role when 0.13.0 deleted the framework status bar, and a "de-emphasized text" color is the global fg token Tuile declines to carry. Carry the shade as a `custom` token instead and render it with `Theme#fg(:hint, text)`, pairing it in a `ThemeDef` so it survives an appearance flip. See `design/decisions.md` `D_no_hint_color`.
- **Breaking:** `lib/tuile/AGENTS.md` and `lib/tuile/component/layout/AGENTS.md` are retired — a namespace inside one gem earns no nested file of its own — and the two that remain trade their per-file lists for a sub-directory map. Their rules already lived in each symbol's rdoc and `design/decisions.md` `D_box_layouts`; `ls` is the file index.
