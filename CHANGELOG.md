...
- Add `Tuile::Canvas#origin` — where a canvas's `(0, 0)` lands in its backend, set by `Screen#canvas_for` to the component's `rect.top_left` and carried through `with(bg_color:)`, so a `repaint` paints in its own coordinates. See `design/decisions.md` `D_canvas`.
- Add `Component#local_rect` / `#local_extent_rect` — `rect` and `extent_rect` with the screen position taken out, which is what a region argument to `canvas.fill` has to be.
...
- **Breaking:** `AbstractStringField#on_escape` arrives with `default_on_escape` already registered instead of assigned. To let ESC bubble again write `field.on_escape.remove(field.method(:default_on_escape))` where you wrote `field.on_escape = nil`.
...
- **Breaking:** A canvas paints in the component's own coordinates — `(0, 0)` is its `rect`'s top-left, not the screen's. Drop the offset from every paint call (`canvas.set_text(rect.left + x, rect.top + y, …)` becomes `canvas.set_text(x, y, …)`) and pass `local_rect` where you passed `rect`; `rect`, `Mouse::Event`, `cursor_position` and a dropdown's anchor are unchanged and still screen-space.
