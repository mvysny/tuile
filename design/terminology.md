...
| **handler** | a `handle_foo` method: the point a subclass *overrides* to react to an event, returning a Boolean claim (`true` = "I took this, stop routing it"). Covers both the routed ones (`handle_key`) and the fan-out hooks (`handle_theme_changed`), which differ only in whether a dispatch mechanism reads the verdict. |
| **listener slot** | an `on_foo=` writer holding a `Proc` an app *assigns* to a stock component (`label.on_theme_changed = …`) — the composition half of the pair whose override half is a *handler*. Always this phrase when the container sense of *slot* is also in play; `on_` names nothing else. |
