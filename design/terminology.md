...
| **row_count** | how many rows the wrapped content occupies. Public on `TextArea` (with `caret_row`, its companion); also on the private `WrappedText`, and the knob on both scrollbars (`VerticalScrollBar#row_count=`, `VerticalScrollBarInk.new(row_count:)`). Not on `TextView` / `List`, which have no caller for it. |
...
| **canvas** | the surface a component paints onto ({Tuile::Canvas}), handed to its `repaint`. Three methods, the component's own (*local*) coordinates, writes outside it dropped. Distinct from the *buffer*, which is the grid a canvas happens to write into. |
| **local** | in a component's **own** coordinates: `(0, 0)` is its top-left. Everything a component deals in — what it paints (`Canvas#origin` puts the canvas here), where its children's `rect`s are measured from, where a `Mouse::Event` counts, what `cursor_position` answers. `Component#local_rect` / `#local_extent_rect` name it, and the prefix is the marker (`D_relative_rect`). |
| **parent space** | where a `rect` is measured: inside the parent, which is that parent's *local* space. So a container divides its `local_rect` and its children answer `rect` in those numbers. |
| **screen space** | absolute terminal coordinates, 0-based from the top-left. What a mouse report carries off the wire, what `Buffer` addresses, and what `absolute_rect` / `absolute_extent_rect` / `to_screen` convert to. |
...
| **transition block** | the optional block a `listener` declaration takes, run on the owning component when its list goes empty↔non-empty — what lets a widget install machinery only while somebody is listening. Not itself a listener, and it has no `on_`/`handle_` name. |
...
| **menu mode** | the {Tuile::Component::MenuBar} state in which a sideways arrow *shows* the neighbouring menu rather than only moving the highlight — `Cascade#browsing?`. Wider than "a panel is open": stepping onto a top-level item with no menu keeps the mode with nothing on screen. |
