...
| **listener slot** | an `on_foo` reader answering a {Tuile::Listeners}, the list of callables an app *registers* on a stock component (`label.on_theme_changed { … }`) — the composition half of the pair whose override half is a *handler*. There is no `on_foo=`: the semantics are append, and remove your own. Use the full phrase wherever the container sense of *slot* is also in play. |
| **event** | a frozen value describing something that happened, passed to every listener of one fire and including the {Tuile::Event} marker. One concept in three namespaces — {Tuile::Mouse}'s wire events, {Tuile::EventQueue}'s loop events, and the ones a *listener slot* fires — with no base class and no member every event must carry. |
