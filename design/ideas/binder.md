...
**after** this lands); `read` shows no verdicts; the empty/`nil` policy; one message per field; buffered,
no model validators past a failing field (unbuffered writes around it); the constants; Vaadin's escape hatches deferred. **Nothing is
...
|---|---|---|
| field validation fails | nothing written; `writeBean` throws, `writeBeanIfValid` → `false` | that field isn't written |
| model validators run | only inside `writeBean` / `writeBeanIfValid` / `writeRecord` | on every field change |
| how they see candidates | write into the bean, run, **revert** on failure | the same, per change |
| `validate()` / `isValid()` | model validators skipped when no bean is set (since 25 `validate()` no longer throws) | all model validators |
...
- **Buffered nests too, if the sub-editor edits the field's *value*, never the model** — a list
  ...
  works. The modes differ in live model validators and live visibility to other components, not in
  nesting.
- **Model validators run Vaadin's way: write into the real model, validate, revert on failure** — snapshot
  the bound attributes through their getters, write the candidates, run the model validators, and on a failure
...
- **Buffered `validate` runs model validators too** — the Binder remembers the model `read` was handed and
...
after `read`), and each API stays small. `bind`, `add_validator`, `last_validation` and the chain live on
the base, `Binder`.
...
- **`Tuile::Binder` is an abstract base both subclass, holding one internal engine** — a
  ...
  else: `bind`, `add_validator`, `last_validation` and their rdoc, which the two classes would otherwise
...
- **`model = nil`** clears every field (`nil` → `empty_value`, the same rule as `read`) and every
  verdict; bindings keep listening and validating, model validators are skipped (no bean), nothing is
...
- **Both classes run a binding whenever its field's value changes** — to refresh that field's
  ...
  model (write-validate-revert, model validators included). `write?` runs every binding plus the model
  validators — that is the "click-gated" part, and it is only the Save gate, not the display.
- **An unbuffered change writes every binding changed since `model=` whose field steps pass**
  ...
  model validator (`end_date` 5) and reverts; `end_date` → 20 then passes against the model's *old* start, and
...
- **…and a failing one sits out rather than holding the rest back — unlike Vaadin.** Vaadin's
  ...
  value that passed, so the model still only ever holds a state the model validators accepted; the divergence
  is on screen, as the field's red verdict. A model validation failure still reverts the whole batch — a model
  validator can't say which attributes it read.
- **Two levels, named as such: *field validation* and *model validation*** — a binding's chain vs. a
  block over the whole model, which the Binder adds with `add_validator { |model| … }`. Not `rule`:
  "name must be 3+ characters" is a rule too, so the word names no level. Not `validate { }`:
  `binder.validate` already runs everything now. Not `with_validator` (Vaadin's): in Ruby `with_x { }`
  reads as a scoped block (`Tuile.without_strict_layout { }`), and Vaadin's returns `this` for a
  chain ours doesn't offer. The binding keeps `.validate { }` — it builds a pipeline beside
  `.convert` and `.required`, where the imperative reads as the step it appends.
- **The cadence is the field's, not the Binder's**, and the Binder owns no blur hook of its own.
  v1 ships against eager fields, accepted: a string or number field with a validator paints its verdict
...
**Staleness rule:** a binding run replaces only its own attr's *field-step* entry; model-validator
entries (the `nil` key, and attrs a model validator blamed) are replaced only when model validators run — in
...
binder.add_validator { |p| "Start date is after end date" if p.start_date && p.end_date && p.start_date > p.end_date }
binder.last_validation  # => {attr => [ValidationFailure]}, frozen; {} when valid
binder.validate         # a full run — every binding + the model validators, every verdict written — → last_validation
...
- **`bind(field, :name)` is `public_send(:name)` / `public_send(:name=, v)`** — attr_accessor,
  ...
  model validator blames. Additive later, but it then owes an answer for both.
...
- **A validator of either level returning anything but a String or `nil` raises `Tuile::Error`** — framework
  ...
  pass every *invalid* value silently. `cond && "msg"` is spelled `"msg" if cond`. (A model validator may
  also return its blame hash.)
...
- **A model validator returns `nil`, a String (form-level), or `{end_date: "…"}` to blame a field**, which
...
- **Buffered, model validators don't run while any field step fails** (Vaadin) — the model would be
  missing that field's candidate, so they would judge a mix of new and stale values. The failing
  field's entry is the whole verdict until it is fixed. An unbuffered edit runs them anyway, with the
  failing field's last good value standing in (above).
- **One message per field: the first failure lands on `error_message`** — `FormItem` has one
  message row (`D_form_item`); a field step contributes one anyway, and when model validators blame an
...
- **The verdict is one map, `{attr => [ValidationFailure]}`** — each a `Data` holding `field`,
  ...
  it; a blamed attribute's candidate), open to more members later. Field steps and model validators fill
  the same map, so the Save alert and the app iterate one thing. Every key holds an array, one
  shape: a field step stops at its first failure, so a field contributes one; several model validators may
  blame the same attr. **Form-level failures sit under the `nil` key** (a `Hash` takes it; Rails'
  `:base` is the fallback that wasn't needed), with `field` and `value` nil. The model validator's
  *return* stays a bare String or hash; the Binder builds the `Data` — it knows no field.
...
(`D_confirm_window`). In write-through mode the model validators have already run live, but Save is still the
