...
set -euo pipefail
# Byte collation throughout: glibc's en_US.UTF-8 ignores `_` at the primary weight, so `sort`
# and `comm` disagree about the order of slug lists and comm cries "not in sorted order".
export LC_ALL=C
