...
module Tuile
  class Component
    ...
    #
    # Lenient in, strict out, so the round trip is inexact for an input that
    # carried a zone:
    #
    #   f.value = Time.now      # takes today's date and the wall clock
    #   f.value == DateTime.now # => false — the offset is gone
    #
    ...
    # half. A save gate on ENTER over a date with no time gets {HasBadInput#bad_input?}
    # and the message; the ink arrives when focus leaves.
    #
    ...
    #   inherits an ancestor's error level; marking only this field would redden
    #   the gap between the halves and leave the halves themselves untouched. So
    ...
    class DateTimeField < Layout::Horizontal
      ...
      # is the half's to ink, on the half's own latch, and a half-filled pair is
      # judged when you leave rather than while you are filling it in.
      #
      # No latch ivar, deliberately: every input here is a fact something
      # announces, which is what makes {#sync_half_wells}'s three call sites
      # complete. A half's `bad_input?` moves with every keystroke and announces
      # nothing at all, so a latch of this field's own could not follow it.
      # @return [Boolean]
